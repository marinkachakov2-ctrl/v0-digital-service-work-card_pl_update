...
export function DiagnosticsSection({
  ...
  // Detect platform on client only to avoid hydration mismatch
  useEffect(() => {
  ...
  const handleEngineHoursPhoto = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!file) return;
      const url = URL.createObjectURL(file);
      const photo: FaultPhoto = {
        id: `ehp-${Date.now()}`,
        url,
        name: file.name,
        timestamp: new Date(),
      };
      onEngineHoursPhotoChange?.(photo);
  ...
  const removeEngineHoursPhoto = useCallback(() => {
    if (engineHoursPhoto) {
      URL.revokeObjectURL(engineHoursPhoto.url);
    }
    onEngineHoursPhotoChange?.(null);
  }, [engineHoursPhoto, onEngineHoursPhotoChange]);
  ...
  return (
    <>
      ...
      {/* Dates and Engine Hours – above diagnostics */}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="hidden"
                  aria-label="Снимка на моточасове"
                />
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        type="button"
                        variant={engineHoursPhoto ? "default" : "outline"}
                        size="icon"
                    ...
                    <TooltipContent>
                      <p>{engineHoursPhoto ? "Смени снимката" : "Задължителна снимка на моточасовете"}</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </div>
              {engineHoursPhoto ? (
                <div className="group relative inline-block h-16 w-24 overflow-hidden rounded-md border border-green-500/40">
                  <img
                    src={engineHoursPhoto.url}
                    alt="Моточасове"
                    className="h-full w-full object-cover"
                  />
                  <button
                    type="button"
                    onClick={removeEngineHoursPhoto}
                ...
                <p className="flex items-center gap-1 text-xs text-destructive">
                  ...
                  Моля, прикачете снимка на моточасовете
                </p>
              ) : null}
            </div>
          </div>
