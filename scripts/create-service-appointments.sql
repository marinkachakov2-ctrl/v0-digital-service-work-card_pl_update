...
-- Drop existing table and views if they exist (for clean re-creation)
DROP VIEW IF EXISTS technician_daily_load;
DROP VIEW IF EXISTS daily_load_stats;
...
CREATE INDEX idx_service_appointments_work_date ON service_appointments(work_date);
CREATE INDEX idx_service_appointments_technician ON service_appointments(technician_name);
...
SELECT 
    ...
    COALESCE(SUM(planned_hours), 0) as reserved_hours,
