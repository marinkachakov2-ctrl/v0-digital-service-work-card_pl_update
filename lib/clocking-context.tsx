...
// Initial sample data - uses a factory to avoid calling Date.now() at module scope (causes hydration mismatch)
function createInitialWorkOrders(): WorkOrder[] {
  const now = Date.now();
  return [
    { 
      id: "WO-5521", 
...
export function ClockingProvider({ children }: { children: React.ReactNode }) {
  const [workOrders, setWorkOrders] = useState<WorkOrder[]>(createInitialWorkOrders);
